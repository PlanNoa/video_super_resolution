...
class VSR(torch.nn.Module):
    ...
    def forward(self, data, target, high_frames, estimated_image, train=True):
        with torch.no_grad():
            ...
            depth_map = interpolate(depth_map.transpose(1, 3).transpose(2, 3), data.shape[2:])
            VOSmask = maskprocess(
                self.VOSModule(data[0].transpose(0, 1).transpose(1, 2), data[1].transpose(0, 1).transpose(1, 2)) != 0)

        masked_estimated_image = torch.tensor(
            [np.ma.MaskedArray(data[1].cpu().numpy(), VOSmask, fill_value=0).filled()], dtype=torch.float32,
