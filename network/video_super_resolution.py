...
class VSR(torch.nn.Module):
    ...
    def forward(self, data, target, high_frames, estimated_image, train=True):
        ...
        data = torch.tensor([up_scaling(img) for img in data])
        optical_flow = torch.tensor([up_scaling(img, shape=data[0].shape) for img in optical_flow])
        depth_map = torch.tensor([up_scaling(img, shape=data[0].shape) for img in depth_map])
